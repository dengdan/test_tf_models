...
def eval_once(saver, summary_writer, corrected, summary_op):
    ...
    with tf.Session(config=util.tf.gpu_config(allow_growth=True)) as sess:
        ...
        try:
            ...
            print('step %r in %s on pvanet, precision = %f ' % (int(global_step), FLAGS.dataset, precision))
...
def evaluate():
    ...
    with tf.Graph().as_default() as g:
        ...
        def tf_get_one_hot():
            ...
            one_hot = tf.py_func(get_one_hot, [num_classes, labels], tf.float32)
            return one_hot
        
        one_hot = tf_get_one_hot() 
        avg_scores = tf.reduce_sum(one_hot* scores)  / tf.cast(labels.shape[-1], tf.float32)
        
        predicted = tf.arg_max(scores, dimension=-1)
        corrects = tf.cast(tf.equal(predicted, tf.cast(labels, tf.int64)), tf.int32)
#         corrects = tf.Print(corrects, [avg_scores])
